...
def get_count_models(user_id):
    ...
    response = requests.get("https://manygpt.onrender.com/api/v1/telegram/models/count", params=params)
...
def get_user_models(user_id, offset, limit):
    ...
    response = requests.get("https://manygpt.onrender.com/api/v1/telegram/models/", params=params)
    if response.status_code != 200:
        return "Проблемы "
    data = response.json()
    bots = []
...
def add_user_model(user_id, model_name, base_model_id, prompt):
    ...
    response = requests.post("https://manygpt.onrender.com/api/v1/telegram/models/new", params=params)
