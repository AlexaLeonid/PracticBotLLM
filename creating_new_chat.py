...
async def food_size_chosen(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=f"Вы выбрали для чата статус {user_data['chosen_name']} и языковую модель {user_data['chosen_llm']}.\n",
...
async def food_size_chosen(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_reply_markup()
    await state.update_data(chosen_chat_name=generate_random_string(8))
    user_data = await state.get_data()
    await callback.message.answer(
